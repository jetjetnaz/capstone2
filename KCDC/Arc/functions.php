...
// INSERT DATA
function insert_shift_schedule($shift_id, $preferred_date, $shift)
{
    global $conn;
    $sql = "INSERT INTO `shifts_schedule` (`shift_id`, `preferred_date`, `shift`) VALUES(?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sss", $shift_id, $preferred_date, $shift);
    return $stmt->execute();
}
...
function insert_archives($shiftID, $shift, $patient_last_name, $patient_first_name, $patient_middle_name, $patient_age,  $patient_gender, $patient_date_birth, $patient_address, $patient_contact_number, $patient_barangay_city, $patient_social_media, $patient_attending_physician, $patient_name_of_companion,  $patient_years_of_dialysis_facility, $patient_frequency_of_treatment, $preferred_date, $patient_hospital_affiliated, $patient_old_dialysis_facility, $patient_relationship_with_patient, $patient_source_funds, $patient_source_income, $patient_source_assistance)
{
    ...
    $sql = "INSERT INTO `archives` (`shiftID`,`shift`, `patient_last_name`, `patient_first_name`, `patient_middle_name`, `patient_age`, `patient_gender`, `patient_date_birth`, `patient_address`, `patient_contact_number`, `patient_barangay_city`, `patient_social_media`, `patient_attending_physician`, `patient_name_of_companion`, `patient_years_of_dialysis_facility`, `patient_frequency_of_treatment`, `patient_preffered_day_treatment`, `patient_hospital_affiliated`, `patient_old_dialysis_facility`, `patient_relationship_with_patient`, `patient_source_funds`, `patient_source_income`, `patient_source_assistance`) VALUES ('$shiftID', '$shift', '$patient_last_name','$patient_first_name', '$patient_middle_name', '$patient_age', '$patient_gender', '$patient_date_birth','$patient_address', '$patient_contact_number', '$patient_barangay_city', '$patient_social_media', '$patient_attending_physician', '$patient_name_of_companion', '$patient_years_of_dialysis_facility', '$patient_frequency_of_treatment','$preferred_date','$patient_hospital_affiliated','$patient_old_dialysis_facility','$patient_relationship_with_patient','$patient_source_funds', '$patient_source_income', '$patient_source_assistance')";
...
{
    global $conn;
    $sql = "UPDATE `shifts_schedule` SET `shift`= ?, `preferred_date` = ?  WHERE `shift_id` = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sss", $shift, $preferred_date, $shiftID);
    return $stmt->execute();
}
